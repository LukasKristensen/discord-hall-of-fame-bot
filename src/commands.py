...
async def get_help(interaction: discord.Interaction):
    ...
    embed.add_field(name="</hide_hof_post_below_threshold>", value="Should hall of fame posts be hidden when they go below the reaction threshold? (Will be visible again when they reach the threshold again)", inline=False)
    ...
    embed.set_image(url="https://raw.githubusercontent.com/LukasKristensen/discord-hall-of-fame-bot/refs/heads/1.14.5/Assets/reaction_calculation_methods.jpg")
    await interaction.response.send_message(embed=embed)
