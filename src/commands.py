...
async def get_help(interaction):
    ...
    embed.add_field(name="/whitelist_emoji", value="Add a whitelisted emoji to the list", inline=False)
    embed.add_field(name="/unwhitelist_emoji", value="Remove a whitelisted emoji from the list", inline=False)
    embed.add_field(name="/get_server_config", value="Get the server configuration", inline=False)
...
async def manual_sweep(interaction, guild_id: int, sweep_limit, sweep_limited: bool, bot: discord.Client,
                       collection, reaction_threshold: int, post_due_date: int, target_channel_id: int, dev_user: int, allow_messages_in_hof_channel: bool):
    """
